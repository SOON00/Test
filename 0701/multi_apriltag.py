...
def detect_apriltags(image):
    ...
    if len(centers) == 4:
        ...
        # 방향 계산
        direction = calculate_direction_from_centers(centers)
        cv2.putText(image, f"Direction: {direction}", (centers[0][0], centers[0][1] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
def calculate_direction_from_centers(centers):
    # 좌우 변의 길이 계산
    left_width = np.linalg.norm(centers[0] - centers[3])
    right_width = np.linalg.norm(centers[1] - centers[2])

    # 좌우 변의 길이의 차이로 방향을 추정
    if abs(left_width - right_width) < 2:  # 길이의 차이가 2 이하이면 "Stay"로 설정
        direction = "Stay"
