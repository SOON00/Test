...
def detect_apriltags(image, depth_frame):
    ...
    distances = []  # 각 AprilTag의 거리를 저장할 리스트
    ...
    # 중심 좌표가 4개인 경우 사각형 그리기 및 평균 거리 계산
    if len(centers) == 4:
        ...
        if len(distances) > 0:
            ...
            cv2.putText(image, f"Avg Distance: {avg_distance:.2f} m", (centers[0][0], centers[0][1] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
