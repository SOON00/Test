...
class AprilTagTfBroadcaster:
    ...
    def get_camera_position(self):
        try:
            # 'camera_link'을 'apriltag_0' 기준으로 가져옵니다.
            (trans, rot) = self.tf_listener.lookupTransform('/apriltag_1', '/camera_link', rospy.Time(0))
            
            # 변환 정보를 출력합니다.
            #rospy.loginfo("Camera Position Relative to AprilTag:")
            #rospy.loginfo("Position (x, y, z): %s", str(trans))
            #rospy.loginfo("Orientation (x, y, z, w): %s", str(rot))
